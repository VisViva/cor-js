...
for (let row_index = -COLUMN_COUNT_HALF; row_index < COLUMN_COUNT_HALF + 1; ++row_index) {
    for (let column_index = 1; column_index < ROW_COUNT; ++column_index) {
        ...
        const material = new Material().filled(true).stroked(true).size(20).width(0.1);
        ...
        for (let keyframe_index = 0; keyframe_index < 5 + Math.random() * 2; ++keyframe_index) {
            ...
            material_keyframes.push(new Keyframe().fill([0, 255, 0, 0]).stroke([0, 0, 0, 0]));
        ...
        if (row_index < -2 || row_index > 2 || column_index < ROW_COUNT / 2 || column_index > ROW_COUNT / 2) {
            material_keyframes.push(new Keyframe().time(10000).fill([0, 0, 0, 0]).stroke([0, 255, 0, 0]));
            ...
            text.scale(1, 1);
            material_keyframes.push(new Keyframe().time(6000).fill([0, 255, 0, 0]).stroke([0, 255, 0, 0]));
            ...
            text_keyframes.push(new Keyframe().time(5000).translateY(- scene._canvas.height / 2 + column_index * VERTICAL_SPACING - scene._canvas.height * 0.9, Easings.cubic, Easings.linear));
            if ((row_index === -2) && (column_index === ROW_COUNT / 2)) {
                material_keyframes.push(new Keyframe().time(6000).fill([0, 255, 0, 0.8]));
...
const rect = new Rect().translateY(- scene._canvas.height + scene._canvas.height * 0.005, Easings.cubic, Easings.linear).width(5 * HORIZONTAL_SPACING).height(scene._canvas.height / 19).material(material);
root.append(rect);
scene.timeline().add(
    rect,
    new Keyframe()
...
scene.timeline().add(
    material,
    new Keyframe().stroke([0, 255, 0, 0]),
    new Keyframe().time(4000).stroke([0, 255, 0, 0]).width(0, Easings.linear, Easings.cubic),
